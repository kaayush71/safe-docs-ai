...
async function fetchAndRedactImages(doc, customPrompt) {
  ...
  const imageFetchPromises = Object.entries(inlineObjects).map(
    async ([objectId, obj]) => {
      const uri =
...
async function applyRedactedImages(doc, redactedImagesMap) {
  for (const [objectId, base64] of redactedImagesMap.entries()) {
    ...
    imageObj.inlineObjectProperties.embeddedObject.imageProperties.contentUri =
      await uploadRedactedImageToDrive(base64, imageObj, accessToken);
  }
}
